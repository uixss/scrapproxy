...
class ProxyScraper:
    def __init__(self, concurrent=500, max_ping=None, country=None, use_geo=False):
        ...
        "http": [
            ...
            "http://proxysearcher.sourceforge.net/Proxy/List.php?type=http",
        ...
        "socks4": [
            ...
            "https://www.my-proxy.com/frsee-socks-4-proxy.html",
        ...
        try:
            async with session.get(url, headers={"User-Agent": generate_user_agent()}, timeout=10) as response:
                ...
                if "application/json" in content_type or text.strip().startswith("{"):
                            ...
                            if isinstance(obj, dict):
                                if "ip" in obj and "port" in obj:
                                    ...
                                    if re.match(r"\d{1,3}(?:\.\d{1,3}){3}", ip) and port.isdigit():
                                        proxies_found.append(f"{ip}:{port}")
                    ...
                    # Primero buscar con REGEX directo (por si están sueltos en texto)
                    for proxy in REGEX.findall(text):
                        self.proxies.append((proxy_type, proxy))

                    # Luego parsear HTML con BeautifulSoup por si están en tablas
                    try:
                        ...
                        for table in tables:
                            for row in table.find_all("tr"):
                                ...
                                for cell in cells:
                                    match = REGEX.search(cell.get_text(strip=True))
                                    if match:
                                        self.proxies.append((proxy_type, match.group()))
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    async def scrape(self):
        ...
        async with aiohttp.ClientSession() as session:
            for ptype, urls in self.proxy_urls.items():
                tasks = [self.fetch(url, session, ptype) for url in urls]
                await asyncio.gather(*tasks)
                await asyncio.sleep(10)  # ⏱️ Espera 10s entre cada grupo de tipo (http, socks4, socks5)
                ...
                async with session.get(test_url, proxy=proxy_url, timeout=8) as res:
                    if res.status == 200:
                        ...
                        if (self.max_ping is None or elapsed <= self.max_ping) and \
                            ...
                            # ✅ Guardar al archivo en tiempo real
...
if __name__ == "__main__":
    ...
    parser.add_argument("-x", "--external", action="store_true", help="Usar proxys.txt existente en vez de hacer scraping")

    args = parser.parse_args()

    # Ajuste automático de concurrencia si no se especifica
    if args.concurrent:
        concurrent = args.concurrent
    else:
        cpu_cores = os.cpu_count() or 4
        concurrent = min(cpu_cores * 200, 5000)
        print(f"⚙️  Concurrencia ajustada automáticamente a {concurrent} según CPU ({cpu_cores} núcleos)")
    ...
    if args.external:
        print("📂 Cargando proxies desde proxys.txt...")
        if not os.path.exists("proxys.txt"):
            print("❌ Archivo proxys.txt no encontrado.")
            exit(1)
        with open("proxys.txt", "r") as f:
            for line in f:
                ...
                scraper.proxies.append((ptype, proxy))
        scraper.proxy_iter = itertools.cycle(scraper.proxies)
    else:
